    ...
    <AppLayout title="Dashboard">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Barang Masuk Index
                <JetNavLink class="float-right" :href="route('posts.create')" v-if="$page.props.bMasuk.create">
...
export default {
  ...
  setup(props) {
    ...
    watchEffect(() => {
      ...
      Inertia.replace(
        route("bMasuk.index", Object.keys(query).length ? query : {})
      );
    });

    const deletePost = (BarangMasukId) => {
      const result = confirm("Apakah anda yakin?");
      if (result) {
        Inertia.delete(route("bMasuk.destroy", BarangMasukId), {
          preserveScroll: true,
        });
      }
    };
